...
| Bug visual | Phase 4 — chapter overlap durante scroll (reportado Rafael 2026-05-14): al hacer scroll desde ch3 hacia adelante, el chapter superior se queda visible encima del siguiente (overlap entre sections). Probable causa: scroll-snap mandatory NO está rompiendo correctamente entre sections cuando el inner content de ch3/ch4 tiene `overflow-y: auto` (D3-12 mobile + ch4 D4-07 overflow:hidden). Hipótesis: scroll-chaining nativo cede al outer snap pero la previous section aún visible debido a height: 100% en .ch{N}-layout. Investigar: verificar `scroll-snap-stop: always` activo en .chapter-section + comprobar z-index/position de .ch{N}-layout (¿position: relative + overflow: hidden está creando un nuevo stacking context que mantiene el chapter visible en el viewport durante el scroll snap?). Repro: `npm run dev`, scroll desde ch3 hacia abajo. | Pending investigation | 2026-05-14 |
| Polish | Phase 4 — Backgrounds downscale ≤80KB cumulative ~1.65MB (ch2-bg.jpg 627KB + ch4-bg-stars-far.jpg 340KB + ch4-bg-planet-mid.jpg 438KB + ch5-hero.jpg 246KB). Bloqueador Phase 6 deploy. 3 opciones: Adobe MCP upload+resize / sharp install / system ImageMagick. | Mandatory pre-Phase 6 deploy | 2026-05-14 |
