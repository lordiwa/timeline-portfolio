...
must_haves:
  truths:
    ...
    - "El checklist cubre las 5 dimensiones que NO se pueden verificar programáticamente: theme bleed visual durante smooth-scroll (THM-04 visual), layout shift al toggle ES↔EN en mobile portrait (I18N-05 + CLS), contrast values reales verificados con axe DevTools / Pa11y / Lighthouse contra los `/* contrast(fg,bg) = X:Y */` documentados inline (THM-05 + A11Y-04), screen reader `<html lang>` reannounce (A11Y-07 best-effort), FOUT/FOIT en primer load (D2-08 fonts pipeline) bajo network throttling, A/B visual 200ms vs 300ms bg morph (Open-Q2-B locked 200ms — re-validar)"
...
**As el** developer (Rafael) cerrando Phase 2 con `/gsd-verify-work 2`, **I want to** un checklist manual estructurado que cubra todo lo que los tests programáticos no pueden verificar (visual diff, perceptual contrast, CLS móvil, FOUT en throttled network, screen reader reannounce), **so that** la phase cierre con confianza visual + a11y + perf antes de pasar a Phase 3.
...
4. **6 secciones de testing**, cada una con descripción + how-to-test + PASS/FAIL criteria + mitigation if FAIL:
   ...
   **Section 5 — FOUT/FOIT en primer load bajo network throttling (D2-08 + R1)**
   - Cobertura: VALIDATION.md row "FOUT/FOIT en primer load"
   - Steps:
   ...
   - PASS: FOUT brief observable + no FOIT + no Lighthouse warning.
   - FAIL: si texto blanco visible mid-load → REPORTAR (probable: algún @fontsource paquete sin `font-display: swap`; verificar el bundle CSS final).
...
6. **Phase 2 Coverage cross-check** — tabla final mapeando cada REQ-ID de Phase 2 a su verification status:
   ...
   | THM-03 | W2 + W4 (themes + fonts) | ✅ automated + manual font visual W5 §6 |
...
<task type="auto">
  ...
  <action>
    ...
    3. **Test Environment block** — bullet list con: browser, mobile emulator, network throttling, PRM emulator, Lighthouse, axe DevTools, NVDA, iOS assumption (deferred — Rafael NO posee iOS device).

    4. **6 sections** (Theme bleed visual / BG morph sync / Layout shift mobile / Contrast audit / FOUT-FOIT / Cross-browser cross-check). Cada section con sub-items numerados (Section 1: 1.1, 1.2, ...; Section 2: 2.1, 2.2, ...; etc.) + criterio PASS/FAIL explícito al final + mitigation if FAIL.
       ...
       - Section 5: FOUT (Flash Of Unstyled Text) es BUENO bajo throttling; FOIT (Flash Of Invisible Text) es MALO.
       - Section 6: tabla de 16 checkpoints de UI-SPEC §14 línea por línea.

    5. **Sign-off block** con checkboxes PASS/FAIL por section + final verdict line.

    6. **Phase 2 Coverage cross-check table** mapeando los 14 REQ-IDs a su verification status (W0..W5).
    ...
    **Estilo de redacción:**
    ...
    - Lista de comandos PowerShell donde aplique (npm run build, npm run preview, DevTools console snippets, Lighthouse CLI optional).

    NO escribir código de tests programáticos (eso se cubre en W0..W4). NO documentar nada sobre Phase 3 (out of scope). Mantener el archivo concentrado en los 4 REQ-IDs manual-only (THM-04 visual, THM-05 external, I18N-05 CLS, A11Y-04 axe audit) + UI-SPEC §14 final E2E checks.

    LOC target: 200-300 líneas (analog 01-06-MANUAL-CHECKLIST.md ~222 LOC).

    Commit message: `docs(qa): plan 06 task 6.1 — Phase 2 manual checklist artifact (W5)`.
  </action>
  <verify>
  ...
  <acceptance_criteria>
    ...
    - Contiene la tabla `Phase 2 Coverage cross-check` con los 14 REQ-IDs (THM-01..05, I18N-01..06, A11Y-03/04/07)
  ...
  <done>02-MANUAL-CHECKLIST.md creado con ≥6 secciones, frontmatter, sign-off block, coverage table, ≥150 LOC.</done>
</task>

<task type="checkpoint:human-verify" gate="blocking">
  <name>Task 6.2: Rafael ejecuta el manual checklist + firma el sign-off</name>
  <what-built>El motor visual + lingüístico completo de Phase 2 está en producción local: 7 themes, BackgroundLayers, LangToggle, 6 fonts self-hosted, i18n end-to-end, ~143 tests verdes, build verde. El manual checklist (Task 6.1) cubre los 4 REQ-IDs manual-only + UI-SPEC §14 final verification.</what-built>
  <how-to-verify>
    **Pre-flight (5 min):**
    1. `cd C:\Users\RafaelMatovelle\documents\mato-new-portfolio`
    2. `npm install` (asegurar deps post-W4)
    3. `npm run test:run` — confirmar ≥143 tests verdes (no regresiones)
    ...
    **Sign-off (5 min):**
    11. En el bloque "Sign-off" del checklist, marcar PASS/FAIL por cada Section 1..6.
    12. Firmar la línea final: "Rafael ejecutó este checklist el {fecha} en {browser+version} {OS+version}. Verdict: PASS/PARTIAL/FAIL".
    13. Si verdict ≠ PASS: añadir entry a STATE.md `Blockers` describiendo qué falló.
    14. Si verdict = PASS: commitear el checklist firmado con `docs(qa): rafael firma 02-MANUAL-CHECKLIST PASS — W5 closes Phase 2 manual gate`.

    **Total estimated time:** 30-40 minutos. Esta es la única wave NO-autonomous de Phase 2 (analog Plan 06 + Plan 07 de Phase 1).
  </how-to-verify>
  <resume-signal>
    Firma con: "approved" + reporte breve (verdict por Section 1..6 + decisión Open-Q2-B 200ms vs 300ms) — entonces yo (el orchestrator) procedo con `/gsd-verify-work 2` para cerrar formalmente la phase.
...
- Esperado: `True` + LOC ≥150 + frontmatter válido + 6 sections + sign-off block + coverage table.
...
<success_criteria>
- 02-MANUAL-CHECKLIST.md creado con estructura completa derivada de VALIDATION.md + UI-SPEC §14
...
- Decisiones tomadas en el checklist (Open-Q2-B final lock, mitigations aplicadas si FAIL)
