...
| 05-05-XX | 05 | 4 | PHA-06 | — | locale watch emite `game.events.emit('locale-changed', locale)`; null-guard `game.value?.events` presente | integration | `npm run test -- tests/phaser/locale-bridge.test.js` | ❌ W0 | ⬜ pending |
| 05-05-XX | 05 | 4 | PHA-07 | — | Planet pointerdown emite `show-project` con id; 3 buttons sr-only en DOM con aria-label disparan mismo handler | integration | `npm run test -- tests/components/Chapter6Content-bridge.test.js tests/a11y/keyboard-planet-buttons.test.js` | ❌ W0 | ⬜ pending |
