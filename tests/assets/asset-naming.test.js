...
//   - 7 busts: ch{0..6}-bust.png
//   - 1 ch2 background: ch2-bg.png
//   - 4 ch4 parallax layers: ch4-bg-stars-far.png, ch4-bg-planet-mid.png,
//     ch4-fg-panels.png, ch4-fg-ships.png
//   - 1 ch5 hero: ch5-hero.png
//
...
const ASSET_NAMING_REGEX =
  /^ch[0-6]-(bust|bg|bg-stars-far|bg-planet-mid|fg-panels|fg-ships|hero)\.png$/

// Defensive: si public/assets/ no existe o no tiene PNGs, devolver lista vacía
// (test setup-friendly antes del primer asset commited).
function pngFilesInAssets() {
  if (!existsSync(ASSETS_DIR)) return []
  return readdirSync(ASSETS_DIR).filter((f) => f.endsWith('.png'))
}

describe('Asset naming convention (ART-05) + privacy gate (D4-02)', () => {
  // T1 — todos los .png matchean enum Phase 4
  it('T1: todos los .png en public/assets/ matchean el enum regex Phase 4', () => {
    const pngs = pngFilesInAssets()
    const offenders = pngs.filter((f) => !ASSET_NAMING_REGEX.test(f))
    expect(
      offenders,
      `Archivos .png con naming fuera de spec Phase 4: ${JSON.stringify(offenders)}. ` +
  ...
  it('T3: no hay .png con underscore en el nombre (ART-05 strict)', () => {
    const offenders = pngFilesInAssets().filter((f) => f.includes('_'))
    expect(
      offenders,
      `Archivos .png con underscore: ${JSON.stringify(offenders)}. ` +
        `ART-05 mandata kebab-case: ch{N}-{descriptor}[-{variant}].png`
    ).toEqual([])
  })
