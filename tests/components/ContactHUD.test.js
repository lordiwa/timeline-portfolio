...
//   T-LOC location (TASK-023 AC5): span.contact-icon renderiza con title + aria-label correctos.
...
//
// TASK-023 review (MEDIUM, comentario de TASK-023): este mock estaba
// desalineado del shape real de contact.js — le faltaba `location` (campo
// TASK-013), así que el <span> de location nunca se renderizaba en NINGÚN
// test de este archivo y su AC5 (location sigue renderizándose) quedaba sin
// lock — la única verificación era una medición CDP manual de una vez. Ver
// T-LOC más abajo.
...
describe('ContactHUD.vue', () => {
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // T-LOC: location span (TASK-013/AC5 TASK-023) — regression lock. Antes de
  // este fix el mock de @/data/contact no traía `location`, así que este
  // render nunca se ejercitaba en la suite (drift preexistente, ver comentario
  // arriba del mock).
  // ─────────────────────────────────────────────────────────────────────────
  it('T-LOC location: span.contact-icon renderiza con title y aria-label correctos', async () => {
    const ContactHUD = (await import('@/components/ContactHUD.vue')).default
    const i18n = createTestI18n({ locale: 'es' })
    const wrapper = mount(ContactHUD, { global: { plugins: [i18n] } })
    const locationSpan = wrapper.find('span.contact-icon')
    expect(locationSpan.exists()).toBe(true)
    expect(locationSpan.attributes('title')).toBe('Quito, Ecuador')
    expect(locationSpan.attributes('aria-label')).toBe('Ubicación de Rafael: Quito, Ecuador')
  })
